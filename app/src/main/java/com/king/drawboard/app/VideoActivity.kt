...
class VideoActivity : AppCompatActivity() {
    ...
    private fun getExternalFilesDir(context: Context): String? {
        ...
        return if (files.isEmpty()) context.getExternalFilesDir("images")?.absolutePath else files[0].absolutePath
        ...
        when (v.id) {
            ...
            R.id.ivSave -> {//保存图片
                lifecycleScope.launch {
                    ...
                    file?.let {
                        Toast.makeText(this@VideoActivity, "保存成功", Toast.LENGTH_SHORT).show()
                    }
                }
            }
